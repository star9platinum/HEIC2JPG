...
macOS 版提供转换脚本 [`src/macos/一键转换HEIC.command`](src/macos/%E4%B8%80%E9%94%AE%E8%BD%AC%E6%8D%A2HEIC.command) 和临时文件清理工具 [`src/macos/清理HEIC2JPG临时文件.command`](src/macos/%E6%B8%85%E7%90%86HEIC2JPG%E4%B8%B4%E6%97%B6%E6%96%87%E4%BB%B6.command)。下载后首次使用时执行：

```bash
chmod +x "一键转换HEIC.command"
chmod +x "清理HEIC2JPG临时文件.command"
```
...
转换脚本会给新临时目录写入项目标记。每张照片完成验证或转换后立即删除对应临时目录；正常退出、转换失败、取消、`Ctrl+C` 或收到终止信号时，也会统一清理当前任务的验证图片、转换中间文件和 `/var/folders` 状态目录。系统崩溃、强制断电或进程被强制结束时仍可能留下目录。

需要彻底清理遗留临时目录时，先确保没有 HEIC2JPG 转换任务正在运行，再双击 `清理HEIC2JPG临时文件.command`：

1. 选择一个或多个曾经转换照片的根目录；取消文件夹选择则只检查当前用户的系统临时目录。
2. 工具递归列出照片目录中的新旧 `.HEIC2JPG` 临时目录，并列出当前用户 `/var/folders` 临时区中的新旧状态目录及估算大小。
3. 输入 `y` 或 `Y` 后彻底删除全部候选；其他输入取消。

清理工具只匹配本项目精确的临时目录命名，不会删除 JPG 或 HEIC 照片。它不会因为目录较新而跳过，所以运行前必须确认转换任务已经停止。
