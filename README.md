...
一键版默认不覆盖已有 JPG；只要存在同名目标，就会在转换前停止并保留全部文件。需要覆盖时请在终端中明确使用 `-f`。符号链接、目录和重复输出映射会在转换前触发安全停止。文件名以 `.` 开头的隐藏 HEIC（包括常见的 `._` AppleDouble 元数据）会被明确列出并跳过，既不转换也不删除。转换失败时会显示具体 HEIC 路径、`sips` 退出码和 macOS 返回的原始诊断信息。
