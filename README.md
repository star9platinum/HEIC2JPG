...
支持 `-q 1..100` 设置 JPG 质量。默认不覆盖已有同名 JPG：脚本会验证该 JPG；只有 JPG 可正常解码且文件修改时间与 HEIC 一致时，才把它作为上次中断留下的结果继续处理。否则保留 HEIC 和现有 JPG、跳过该文件并继续处理其他照片。明确使用 `-f` 时会重新转换并替换已有的普通 JPG 文件。
...
5. 脚本重新检查 HEIC 文件集合、成功项的源文件稳定快照、JPG 哈希和可解码性；只删除验证成功项对应的原图。

断点续传复用已有 JPG 时，修改时间一致只是识别本脚本历史输出的安全条件之一，不能证明两张照片内容相同，因此仍必须在 Finder 中人工检查。多选目录相互重叠时，同一 HEIC 只处理一次。符号链接、目录和重复输出映射会在转换前触发安全停止。文件名以 `.` 开头的隐藏 HEIC（包括常见的 `._` AppleDouble 元数据）会被明确列出并跳过，既不转换也不删除。转换失败时会显示具体 HEIC 路径、`sips` 退出码和 macOS 返回的原始诊断信息。
